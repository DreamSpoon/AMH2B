...
from .geo_nodes import (SIGNED_NEAREST_GEO_NG_NAME, SB_WEIGHT_GEO_NG_NAME, create_geo_ng_signed_nearest,
    create_weight_sb_mod_geo_node_group)
...
def create_weighting_object(context, override_create, recv_ob, send_ob, goal_vg_name, mask_vg_name, mass_vg_name):
    ...
    create_weight_test_mat_nodes(weight_test_mat, goal_vg_name, mask_vg_name, mass_vg_name)
...
def data_transfer_sb_weights(context, ob, other_ob, vert_mapping, individual, apply_mod, include_goal,
        include_mask, include_mass, goal_vg_name, mask_vg_name, mass_vg_name):
    if individual:
        mod_created = False
...
def preset_soft_body(ob, goal_vg_name, mass_vg_name):
